...
<title>OGP Ingest | Ingest WMS GetCapabilities</title>
</head>
<body>
...
<h3>Ingest WMS GetCapabilities</h3>
<p>
Use this page to ingest from a WMS GetCapabilities document.
</p>

		<form id="fileuploadForm" action="ingestFromWMS" method="POST" class="cleanform">
			<div class="header">
		  		<h2>Enter a WMS endpoint and the institution records should be posted to.</h2>
			</div>
				<%@include file="jspf/institutions.jspf"%>

			<label for="wmsEndpoint">WMS Endpoint:</label>
			<input type="text" id="wmsEndpoint" name="wmsEndpoint" />
		...
		$(document).ready(function() {
			$("#wmsCapabilitiesNav").addClass("active");
			...
			pollIngestStatus = function(jobId){
				jQuery.getJSON("ingestStatus?jobId=" + jobId, function(data){
					...
					if (currentStatus == "Processing"){
						...
						$("#loader").html("");
						jQuery("#status").prepend("<span id='finished'>Finished!</span>");
					}
