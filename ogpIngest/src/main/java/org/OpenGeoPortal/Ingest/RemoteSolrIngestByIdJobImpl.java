...
public class RemoteSolrIngestByIdJobImpl implements RemoteSolrIngestByIdJob, Runnable {
	...
	private void getSolrRecords() throws MalformedURLException{
		...
		do {
			...
			try{
				...
				ingestStatus.addError("solrQuery  [" + queryString + "]", "query failed");
			}
			if (totalCount == 0){
			...
			try{
				...
				//if(!solrdocs.isEmpty()){
					try {
						...
						ingestStatus.addError("solrQuery  [" + queryString + "]", "Error setting beans" + e.getMessage());
					}
				}
			} catch (Exception e){
				logger.error("error getting beans" + e.getMessage());	
				ingestStatus.addError("solrQuery  [" + queryString + "]", "Error getting beans" + e.getMessage());
		...
		ingestStatus.addError("solrQuery  [layerIds]", "Unknown Exception: " + e.getMessage());
	}
	...
	public void init(UUID jobId, String[] layerIds, String remoteSolrUrl) {
		this.jobId = jobId;
